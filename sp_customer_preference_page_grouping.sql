/****** Object:  StoredProcedure [consumption].[sp_customer_preference_page_grouping]    Script Date: 12/2/2022 6:42:18 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE  OR ALTER  PROCEDURE [consumption].[sp_customer_preference_page_grouping]
(
...
DECLARE @TranName VARCHAR(20);  
		SELECT @TranName = 'customer_preference_page_grouping';  
		BEGIN TRY
...
delete from [consumption].[customer_preference_page_grouping] where year(@report_date)=report_year and month(@report_date)=report_month ;
insert into [consumption].[customer_preference_page_grouping]
Select month(@report_date) as report_month,
year(@report_date) as report_year,
A.page_grouping as page_grouping,
...
min(visit_date) as visit_date from [adobe].[app_lm1_lm2_silver]  where page_grouping is NOT NULL and  visit_date<=@report_date and visit_date>=EOMONTH(DATEADD(Month,-13,@report_date))
group by visit_id,visitor_id,page_grouping)A
left join
(select member_id,  
...
group by B.place_of_residence,B.is_new_member,C.Membership_Tier,A.is_member,A.page_grouping;
--==========================================
--==== Error Handling 
--==========================================
